...
public class CJMachineWhooperOld implements CJIMachine {
	...
	public void updateMachine(CJTileEntityMachineBase machineEntity) {
		...
		for(int i = 0; i < adjacentMachines.length; ++i) {
			...
			if(adjacentMachine != null) {
				...
				slotIndex = CJInventoryHelper.getMatchingOutputIndex(
		...
		for(int i = 0; i < adjacentMachines.length; ++i) {
			...
			if(adjacentMachine != null) {
				...
				slotIndex = CJInventoryHelper.getMatchingInputIndex(
						adjacentChest, currentItemID);
